...
def Home(request):
    ...
    profile_pic_url = user.profile_pic.url if user.profile_pic else None # Get the profile picture URL
    ...
    if request.method == "POST":
        ...
        if CustomUser.objects.filter(username=username).exists():
            ...
            if cropped_image_data:
                ...
                format, imgstr = cropped_image_data.split(';base64,')  # Split base64 string
                ext = format.split('/')[-1]  # Get file extension (e.g., jpeg, png)
                pic = ContentFile(base64.b64decode(imgstr), name=f"profile_pic.{ext}")
...
def NOTES_DETAILS(request):
    ...
    subject_filter = request.GET.get('subject', '')  # Get selected subject filter

    # Filter notes based on both search query and selected subject
    if query and subject_filter:
        data_list = Notes.objects.filter(subject__icontains=query, subject__iexact=subject_filter)
    elif query:
        data_list = Notes.objects.filter(subject__icontains=query)
    elif subject_filter:
        data_list = Notes.objects.filter(subject__iexact=subject_filter)
    else:
...
def HOME_NOTES_DETAILS(request):
    query = request.GET.get('q', '')  # Get search query
    subject_filter = request.GET.get('subject', '')  # Get selected subject filter
    ...
    return render(request, 'homenotes.html', context)
